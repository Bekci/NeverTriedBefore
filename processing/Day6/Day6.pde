...
class FractalRoot {
  PointObj[] pointArr = new PointObj[7];
  Branch rootBranch;
FractalRoot() {
  float centX = width/2; 
  float centY = height/2;
  int count = 0;
  for (float i = 0; i<359; i+=float(360)/7) {
    float x = centX + (1600 * cos(radians(i)));
    float y = centY + (1600 * sin(radians(i)));
    print(i," ");
...
class Branch {
  ...
  Branch(int lev, int n, PointObj[] points) { 
    ...
    if ((level+1) < _maxlevels) {
      ...
      childBranch = new Branch(level+1, k+1, newPoints); myBranches = (Branch[])append(myBranches, childBranch);
      }
    }
  ...
  void drawMe() {
    ...
    stroke(0,50);
    beginShape();
    curveVertex((outerPoints[outerPoints.length-1].x+outerPoints[0].x)/2, (outerPoints[outerPoints.length-1].y+outerPoints[0].y)/2);
    for (int i = 0; i < outerPoints.length; i++) {
      ...
      curveVertex((outerPoints[i].x+outerPoints[nexti].x)/2, (outerPoints[i].y+outerPoints[nexti].y)/2);
    }
    curveVertex((outerPoints[outerPoints.length-1].x+outerPoints[0].x)/2, (outerPoints[outerPoints.length-1].y+outerPoints[0].y)/2);
    curveVertex(outerPoints[0].x, outerPoints[0].y);
    curveVertex((outerPoints[0].x+outerPoints[1].x)/2, (outerPoints[0].y+outerPoints[1].y)/2);
    endShape();
    strokeWeight(0.5);
    fill(255, 150);
